...
def fetch_from_mssql() -> pd.DataFrame:
    ...
    SELECT
        ...
        target_date,
        CAST(yhat AS decimal(18,6)) AS yhat,
        model_version,
        run_id,
        created_at
    FROM dw.fact_forecast_daily;
    """
...
def load_into_pg_staging(df: pd.DataFrame) -> int:
    ...
    df = df.copy()
    df["run_id"] = df["run_id"].apply(to_uuid_or_none)
    ...
    with psycopg2.connect(**build_pg_conn_kwargs()) as pg:
        with pg.cursor() as cur:
            cur.execute("TRUNCATE TABLE integration.stg_fact_forecast_daily;")

            execute_values(
                cur,
                """
                INSERT INTO integration.stg_fact_forecast_daily
    ...
    print(f"[OK] Loaded {len(rows)} rows into integration.stg_fact_forecast_daily.")
    return len(rows)
...
def main() -> None:
    ...
    df = fetch_from_mssql()
    print(f"[INFO] Fetched {len(df)} rows from MSSQL dw.fact_forecast_daily.")
